...
bitfinex_depos_and_withdrawals %<>% #add column for matching ccs next
  merge(x = .,
        y = my_bitfinex_ccs[ , c("Bitfinex_Ticker", "four_letter_ticker")],
        by.x = "CURRENCY",
        by.y = "Bitfinex_Ticker",
        all.x = T)

bitfinex_dw_column_order %<>% c(., "four_letter_ticker")
...
unique_ccs_in_bitfinex <- #create vector for matching
  unique(bitfinex_depos_and_withdrawals$four_letter_ticker) %>%
  sort

ccs_in_bitfinex_not_matched_in_ref_table <-
  unique_ccs_in_bitfinex[unique_ccs_in_bitfinex %not_in% names(refTable)] %>%
  sort


#Simple test to make sure all of your CCs are accounted for
if(length(ccs_in_bitfinex_not_matched_in_ref_table) == 0) {
  svDialogs::dlgMessage("All CCs accounted for. You should proceed.")
} else {
  svDialogs::dlgMessage("All CCs are NOT accounted for; check the four_letter_tickers.")
  stop()
}

#Simple test to see if any missing four_letter_tickers
if(!any(is.na(bitfinex_depos_and_withdrawals$CURRENCY))) {
  svDialogs::dlgMessage("No missing, you should proceed.")
} else {
  svDialogs::dlgMessage("Some four_letter_ticker data are missing; check them and fix before moving on.")
  stop()
}
...
bitfinex_depos_and_withdrawals$SEK_rate <- #lookup values in refTable. Cannot use "$Date" since cbind converts "date" classes and the lookup will fail.
  refTable[cbind(
    ...
    bitfinex_depos_and_withdrawals$four_letter_ticker)] %>%
  as.numeric
...
bitfinex_trades %<>% #add columns for matching ccs next
  merge(x = .,
        ...
        by.x = "Start",
        by.y = "Bitfinex_Ticker") %>%
  dplyr::rename(Start_four_letter_ticker = four_letter_ticker) %>%
  merge(x = .,
        y = my_bitfinex_ccs[ , c("Bitfinex_Ticker", "four_letter_ticker")],
        by.x = "End",
        by.y = "Bitfinex_Ticker") %>%
  dplyr::rename(End_four_letter_ticker = four_letter_ticker) %>%
  merge(x = .,
        y = my_bitfinex_ccs[ , c("Bitfinex_Ticker", "four_letter_ticker")],
        by.x = "FEE CURRENCY",
        by.y = "Bitfinex_Ticker") %>%
  dplyr::rename(Fee_four_letter_ticker = four_letter_ticker)

bitfinex_trades_column_order %<>% c(., "Start_four_letter_ticker", "End_four_letter_ticker", "Fee_four_letter_ticker")
...
unique_ccs_in_bitfinex <- #create vector for matching
  unique(c(bitfinex_trades$Start_four_letter_ticker, bitfinex_trades$End_four_letter_ticker, bitfinex_trades$Fee_four_letter_ticker)) %>%
  sort
...
bitfinex_trades$Start_SEK_rate <- #lookup values in refTable. Cannot use "$Date" since cbind converts "date" classes and the lookup will fail.
  refTable[cbind(
    ...
    bitfinex_trades$Start_four_letter_ticker)] %>%
  as.numeric
...
bitfinex_trades$End_SEK_rate <- #lookup values in refTable. Cannot use "$Date" since cbind converts "date" classes and the lookup will fail.
  refTable[cbind(
    bitfinex_trades$Date_as_character,
    bitfinex_trades$End_four_letter_ticker)] %>%
  as.numeric
...
bitfinex_trades$Fee_SEK_rate <- #lookup values in refTable. Cannot use "$Date" since cbind converts "date" classes and the lookup will fail.
  refTable[cbind(
    bitfinex_trades$Date_as_character,
    bitfinex_trades$Fee_four_letter_ticker)] %>%
  as.numeric
...
gatehub_balance_changes %<>% #add column for matching ccs next
  merge(x = .,
        y = my_gatehub_ccs[ , c("Gatehub_Ticker", "four_letter_ticker")],
        by.x = "Currency",
        by.y = "Gatehub_Ticker",
        all.x = T)

gatehub_column_order %<>% c(., "four_letter_ticker")
...
unique_ccs_in_gh <- #create vector for matching
  unique(gatehub_balance_changes[["four_letter_ticker"]]) %>%
  sort

ccs_in_gh_not_matched_in_ref_table <-
  unique_ccs_in_gh[unique_ccs_in_gh %not_in% names(refTable)] %>%
  sort
...
gatehub_balance_changes[["SEK_rate"]] <- #lookup values in refTable. Cannot use "$Date" since cbind converts "date" classes and the lookup will fail.
  refTable[cbind(
    ...
    gatehub_balance_changes[["four_letter_ticker"]])] %>%
  as.numeric
...
ccs_in_gatehub <- gatehub_balance_changes[["four_letter_ticker"]] %>% unique
...
for (i in 1 : length(ccs_in_gatehub)) {
  ...
  ith_cc_df <-
    sqldf::sqldf(paste0('
                        ...
                        where four_letter_ticker = "', ccs_in_gatehub[i], '"
                        '))
  
  ith_cc_df %<>% dplyr::arrange(Date_time)
...
gatehub_balance_changes %<>% dplyr::arrange(Date_time)
...
transferwise_transfers %<>% #Drop columns that aren't needed
  .[.$Status == "transferred", ] %<>%
  .[(.$Reference %not_in% c('UPS packag', 'VERIFYBANK')), ]
...
transferwise_transfers %<>% #add columns for matching CCs next
  merge(x = .,
        y = my_transferwise_ccs[ , c("Transferwise_Ticker", "four_letter_ticker")],
        by.x = "Start",
        by.y = "Transferwise_Ticker") %>%
  dplyr::rename(Start_four_letter_ticker = four_letter_ticker) %>%
  merge(x = .,
        y = my_transferwise_ccs[ , c("Transferwise_Ticker", "four_letter_ticker")],
        by.x = "End",
...
transferwise_column_order %<>% c(., "Start_four_letter_ticker", "End_four_letter_ticker")
...
if(sum(rows_without_txid != rows_without_balance) == 0) {
  svDialogs::dlgMessage("Can use either 'rows_without_txid' or 'rows_without_balance' for subsetting. You should proceed.\n\n")
} else {
  svDialogs::dlgMessage("The two lists are not identical - you must understand why and fix it before proceeding.")
  stop()
}

rows_with_txid <-
  which(row(df_for_testing_uniqueness_of_kraken_rows) %not_in%
          rows_without_txid)
...
columns_to_concatenate <- #timestamps vary on some of these rows although all other data seems to be identical
  colnames(df_for_testing_uniqueness_of_kraken_rows)[
    colnames(df_for_testing_uniqueness_of_kraken_rows) %not_in%
      c("txid", "balance", "time")]
...
if(!(any(vector_of_strings_without_txid %not_in% vector_of_strings_with_txid))) {
  svDialogs::dlgMessage("Information is duplicated when txid is blank and those rows should be ignored.")
} else {
  svDialogs::dlgMessage("It is unclear which rows should be ignored. You must explore the data more before proceeding.")
  stop()
}
...
kraken_ledgers %<>% #add column for matching ccs next
  merge(x = .,
        y = my_kraken_ccs[ , c("Kraken_ledger_ticker", "four_letter_ticker")],
        by.x = "asset",
        by.y = "Kraken_ledger_ticker",
        all.x = T)

kraken_column_order %<>% c(., "four_letter_ticker")
...
unique_ccs_in_kraken <- #create vector for matching
  unique(kraken_ledgers$four_letter_ticker) %>%
  sort

ccs_in_kraken_not_matched_in_ref_table <-
  unique_ccs_in_kraken[unique_ccs_in_kraken %not_in% names(refTable)] %>%
  sort
...
#Simple test to see if any missing four_letter_tickers
if(!any(is.na(kraken_ledgers$four_letter_ticker))) {
  svDialogs::dlgMessage("No missing, you should proceed.\n\n")
} else {
  svDialogs::dlgMessage("Some four_letter_ticker data are missing; check them and fix before moving on.")
  stop()
...
kraken_ledgers[["SEK_rate"]] <- #lookup values in refTable. Cannot use "$Date" since cbind converts "date" classes and the lookup will fail.
  refTable[cbind(
    ...
    kraken_ledgers[["four_letter_ticker"]])] %>%
  as.numeric
...
bitfinex_for_skatteverket_01_depos_and_withdrawals$Ticker_Beteckning <-
  bitfinex_for_skatteverket_01_depos_and_withdrawals$four_letter_ticker
...
bitfinex_for_skatteverket_02_trades_fees$Ticker_Beteckning <-
  bitfinex_for_skatteverket_02_trades_fees$Fee_four_letter_ticker
...
bitfinex_for_skatteverket_02_trades_start$Ticker_Beteckning <-
  bitfinex_for_skatteverket_02_trades_start$Start_four_letter_ticker
...
bitfinex_for_skatteverket_02_trades_end$Ticker_Beteckning <-
  bitfinex_for_skatteverket_02_trades_end$End_four_letter_ticker
...
transferwise_for_skatteverket$Action_Händelse <- 
  ifelse(transferwise_for_skatteverket$Start_four_letter_ticker == "SEK_",
         "Buy",
         "Sell")
...
gatehub_for_skatteverket$Ticker_Beteckning <-
  gatehub_for_skatteverket$four_letter_ticker
...
kraken_for_skatteverket$Ticker_Beteckning <-
  kraken_for_skatteverket$four_letter_ticker
...
all_trades_all_years %<>%
  dplyr::arrange(Action_Händelse) %<>%
  dplyr::arrange(Transaction_Datum)

...
if(!dir.exists("../outputs/05_cmc")) {
  dir.create("../outputs/05_cmc")
}

#For all days of interest will do a loop to capture the opening prices for the day of interest (note that the website may show the previous date in the header, which is why these are taken as opening prices)
for (i in 1 : length(days_for_coinmarketcap)) {
  ...
  ifelse(test = file.exists(file_path),
         {cat(paste0("File ", file_path, " exists. Skipping to next iteration.\n\n"))
           next},
         
         #If the data has not already been scraped, will scrape and save it
         {cat(paste0("File ", file_path, " not found. Navigating to page to scrape data.\n\n"))
             ...
             tryCatch(
               {
                 ...
                 load_more_button <- rD$client$findElement(using = "css selector", value = ".cmc-table-listing__loadmore .bzWQIF")
           ...
           #After all data has been loaded to the page, the page should be scraped

...
Rate_to_find_deductableportionof_8.4 <- openxlsx::read.xlsx("../inputs/09_skatteverket_parameters.xlsx")[["Rate_to_find_deductableportionof_8.4"]]

Taxrate_on_profits <- openxlsx::read.xlsx("../inputs/09_skatteverket_parameters.xlsx")[["Taxrate_on_profits"]]

#Pull in a ref table to indicate how to treat different assets in terms of Swedish tax rules
ref_table %<>% .[ , c("Ticker_to_report",
                      "Profitbox_Vinstruta",
                      "Lossbox_Förlustruta")]
...
for (year in first_year : final_year) {
  ...
  k4_c_for_fiat_or_d_for_crypto_table <-
    sqldf::sqldf("
                 ...
                 join ref_table as r
                 on s.Ticker_Beteckning = r.Ticker_to_report") %>%
    subset(select = -c(Ticker_to_report))
